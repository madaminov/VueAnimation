Vue.component('widget', {
  name: 'Widget',
  template: `<transition-group name="cards" tag="div" :class="['wrapper-item', { 'wrapper-item-open': selected != (null) }]" >
            <!--item-->
            <div v-for="(card,index) in cards" :key="card.id" :class="['item', 'item-' + (index), { 'is-open': open === (index) }]"
                :style="itemStyle(index)" @click="select(index)" :aria-pressed="selected === (index)" role="button"
                tabindex="index">
                <transition name="fade-up-button">
  ...
  data: function () {
    return {
      ...
      selected: null,
      open: null,
      bottomSlots: ['0%', 'calc(33% + 12px)', 'calc(66% + 20px)'], // тут position left
      topY: '0', // тут position top
      bottomY: '0', // тут position bottom
    };
  },
  ...
  methods: {
    ...
    itemStyle(i) {
      const isTop = this.selected === i;
      let left = this.bottomSlots[i];
      if (
        this.selected != null &&
        this.selected != i &&
        this.selected != this.cards.length - 1
      ) {
        left = this.bottomSlots[i - 1];
      }
      let top = this.topY;
      let bottom = 'auto';
